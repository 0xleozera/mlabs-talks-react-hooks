...
const useFollowers = () => {
  ...
  const normalizeUserFollowers = useCallback((followers) => {
    const mappedUserFollowers = followers.map(
      ({ login: username, avatar_url: avatarUrl }) => ({ username, avatarUrl })
    );

    return mappedUserFollowers;
  }, []);

  const saveUserFollowers = useCallback(
    (followers) => {
  ...
  const fetchUserFollowers = useCallback(async () => {
    const request = await fetch(
      "https://api.github.com/users/leonardorpr/followers?page=1&per_page=5"
    );
